          ...
          <div class="emotion-intensity">
            <span class="intensity-dots">
              <span
                ...
                class="dot"
                :class="{
                  active: getIntensityClass(currentEmotion.emtionScore) >= dot,
                }"
              ></span>
            </span>
...
const iconUrl = new URL("@/assets/images/run_login.png", import.meta.url).href;
...
const createNewFrontendSession = () => {
  //创建新的会话对象
  const newSession = {
    sessionId: `temp_${Date.now()}`, //字符串加返回当前的时间这是唯一的标识
    status: "TEMP", //临时会话
    sessionTitle: "新对话",
  };
...
const loadSessionEmotion = (sessionId) => {
  ...
  getSessionEmotion(id).then((res) => {
    console.log(res);
    currentEmotion.value = res; //将后端返回的情绪分析结果赋值给currentEmotion
  });
};
...
const startNewSession = (message) => {
  // 构建会话参数
  const sessionParams = {
    initialMessage: message, //用户发送的message=初始消息
  };
  ...
  startSession(sessionParams).then((res) => {
    ...
    if (currentSession.value && currentSession.value.status === "TEMP") {
      //更新数据
      Object.assign(currentSession.value, sessionData); //合并对象
    } else {
      //如果不是临时会话，创建新的会话
...
const startAIResponse = (sessionId, userMessage) => {
  ...
  fetchEventSource("/api/psychological-chat/stream", {
    ...
    onmessage: (event) => {
      ...
      // 当前会话的AI消息
      const aiMessage = messages.value[messages.value.length - 1]; //获取最后一个消息
      ...
      const payload = JSON.parse(raw); //将字符串转换为对象
...
const handleSessionClick = (session) => {
  ...
  getSessionEmotion(session.id).then((res) => {
    console.log(res);
    messages.value = res;
  });
